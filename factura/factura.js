...
function crearLinia(linia) {
    ...
    descompte.addEventListener('input',()=>aplicarDescompte(id,input.value, linia.Article.preu));
    let botoPositiu = document.createElement('button');
    let botoNegatiu = document.createElement('button');
    botoPositiu.addEventListener('click', ()=>afegirUn(id,input.value, linia.Article.preu));
    botoPositiu.innerHTML = '+';
    botoNegatiu.innerHTML = '-';
    botoNegatiu.addEventListener('click', ()=>restarUn(id,input.value,linia.Article.preu));
    ...
    cell3.appendChild(botoPositiu);
    cell3.appendChild(botoNegatiu);
    cell3.appendChild(descompte);
...
function afegirUn(id, quantitat,preu) {
    let table = document.getElementsByTagName("table")[0];
    let trs = table.getElementsByTagName("tr");
    let total = 0;
    if(quantitat > 0){
...
function restarUn(id,quantitat,preu) {
    let table = document.getElementsByTagName("table")[0];
    let trs = table.getElementsByTagName("tr");
    let total = 0;
    if(quantitat > 0){
...
function aplicarDescompte(id,quantitat,preu){
    let table = document.getElementsByTagName("table")[0];
    let trs = table.getElementsByTagName("tr");
    let total = 0;
    for(let i = 1; i < trs.length; i++){
        let linia = trs[i].getElementsByTagName("th");
        if(trs[i].id === id){
            let descompte = linia[2].getElementsByTagName("input")[1].value;
            linia[4].innerHTML = calcularTotal(quantitat, preu - (preu * descompte / 100));
        }
        total += parseInt(linia[4].innerHTML);
    }
    baseImposable(total);
